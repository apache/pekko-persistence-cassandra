# Agent Rules for Apache Pekko Persistence Cassandra
...
- Cassandra schema changes must include migration notes and backward compatibility considerations.
- DSE (DataStax Enterprise) compatibility must be preserved for dseTest.
...
```shell
sbt "endToEndExample / run"
```

- Use JDK-specific configs when relevant.

```shell
sbt "core / TestJdk9 / testOnly fully.qualified.SpecName"
```
...
- Cassandra integration tests require a running Cassandra cluster. If unavailable, record in `Tests`.
...
- CQL queries must be parameterized to prevent injection. Never concatenate user input into CQL strings.
