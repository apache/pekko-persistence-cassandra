...
class EventsByTagSpec extends AbstractEventsByTagSpec(EventsByTagSpec.config) {
  ...
  "Cassandra live eventsByTag" must {
    ...
    "find new events" in {
      ...
      withProbe(queries.eventsByTag(tag = "black", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(2)
    ...
    "find events from timestamp offset" in {
      withProbe(
        queries.eventsByTag(tag = "green", offset = NoOffset).runWith(TestSink[Any]()),
        probe1 => {
          probe1.request(2)
          val appleOffs = probe1
            .expectNextPF {
          ...
          withProbe(
            queries.eventsByTag(tag = "green", queries.timeBasedUUIDFrom(bananaTimestamp)).runWith(TestSink[Any]()),
            probe2 => {
              probe2.request(10)
              if (appleTimestamp == bananaTimestamp)
                probe2.expectNextPF { case e @ EventEnvelope(_, "a", 2L, "a green apple") => e }
    ...
    "find events from UUID offset " in {
      withProbe(queries.eventsByTag(tag = "green", offset = NoOffset).runWith(TestSink[Any]()),
        probe1 => {
          probe1.request(2)
          probe1.expectNextPF { case e @ EventEnvelope(_, "a", 2L, "a green apple") => e }
          val offs = probe1.expectNextPF { case e @ EventEnvelope(_, "a", 4L, "a green banana") => e }.offset
    ...
    "find new events that spans several time buckets" in {
      ...
      withProbe(queries.eventsByTag(tag = "T1-live", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(10)
          probe.expectNextPF { case e @ EventEnvelope(_, "p1", 1L, "e1") => e }
          probe.expectNextPF { case e @ EventEnvelope(_, "p1", 2L, "e2") => e }
    ...
    "sort events by timestamp" in {
      ...
      withProbe(queries.eventsByTag(tag = "T2", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(10)
    ...
    "stream many events" in {
      ...
      withProbe(
        queries.eventsByTag(tag = "yellow", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
    ...
    "not complete for empty query" in {
      val probe = queries.eventsByTag(tag = "empty", offset = NoOffset).runWith(TestSink[Any]())
      probe.request(2)
      probe.expectNoMessage(waitTime)
      probe.cancel()
    }
...
class EventsByTagZeroEventualConsistencyDelaySpec
    extends AbstractEventsByTagSpec(
      ConfigFactory.parseString("""
            pekko.persistence.cassandra.query.eventual-consistency-delay = 0s
...
pekko.persistence.cassandra.events-by-tag.new-persistence-id-scan-timeout = 600ms
...
""")
  ...
  "Cassandra live eventsByTag delayed messages" must {
    ...
    "find delayed events" in {
      ...
      withProbe(queries.eventsByTag(tag = "T6", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(10)
          probe.expectNextPF { case e @ EventEnvelope(_, "p1", 1L, "p1-e1") => e }
          probe.expectNextPF { case e @ EventEnvelope(_, "p2", 1L, "p2-e1") => e }
    ...
    "find delayed events 2" in {
      ...
      withProbe(queries.eventsByTag(tag = "T7", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(10)
    ...
    "find delayed events 3" in {
      ...
      withProbe(queries.eventsByTag(tag = "T8", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(10)
    ...
    "find delayed events from offset" in {
      ...
      withProbe(queries.eventsByTag(tag = "T9", offset = NoOffset).runWith(TestSink[Any]()),
        probe1 => {
          probe1.request(10)
          val offs =
            probe1.expectNextPF { case e @ EventEnvelope(_, "a", 1L, "A1") => e }.offset.asInstanceOf[TimeBasedUUID]

          withProbe(queries.eventsByTag(tag = "T9", offset = offs).runWith(TestSink[Any]()),
            probe2 => {
              probe2.request(10)
    ...
    "find delayed events when many other events" in {
      ...
      withProbe(queries.eventsByTag(tag = "T10", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(1000)
    ...
    "find events from many persistenceIds" in {
      ...
      withProbe(
        queries.eventsByTag(tag = "T11", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(1000)
...
class EventsByTagStrictBySeqNoEarlyFirstOffsetSpec
  ...
  "Cassandra live eventsByTag with delayed-event-timeout > 0s and firstOffset = 1000 days ago" must {
    "find all events when starting the query 1000 days ago" in {
      ...
      withProbe(queries.eventsByTag(tag = "T11", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(2000)
...
class EventsByTagLongRefreshIntervalSpec
  ...
  "only look for new-persistence-id timeout for previous events for new persistence ids" in {
    ...
    val offset: Offset =
      withProbe(queries.eventsByTag(tag = "animal", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
          probe.request(2)
          probe.expectNextPF {
    ...
    withProbe(queries.eventsByTag(tag = "animal", offset = offset).runWith(TestSink[Any]()),
      probe => {
        probe.request(2)
...
class EventsByTagStrictBySeqMemoryIssueSpec extends AbstractEventsByTagSpec(EventsByTagSpec.strictConfig) {
  ...
  "Cassandra eventsByTag with many events" must {
    ...
    "not use more than then buffer capacity when looking for delayed" ignore {
      ...
      withProbe(
        queries.eventsByTag(tag = "T13", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
    ...
    "not use more than then buffer capacity when looking for missing" ignore {
      ...
      withProbe(queries.eventsByTag(tag = "T14", offset = NoOffset).runWith(TestSink[Any]()),
        probe => {
...
class EventsByTagSpecBackTrackingLongRefreshInterval
    extends AbstractEventsByTagSpec(
      ConfigFactory.parseString("""
    pekko.persistence.cassandra.query.refresh-interval = 10s
    pekko.persistence.cassandra.events-by-tag {
     back-track {
        interval = 500ms
        period = max
...
//  pekko.persistence.cassandra.query.refresh-interval = 4s
    pekko.persistence.cassandra.events-by-tag {
     back-track {
        interval = 1s
        period = 10m
...
class EventsByTagPersistenceIdCleanupSpec extends AbstractEventsByTagSpec(EventsByTagSpec.persistenceIdCleanupConfig) {
  ...
  "PersistenceId cleanup" must {
    "drop state and trigger new persistence id lookup periodically" in {
      ...
      // Use a unique pid per test run to prevent events written in one attempt from
