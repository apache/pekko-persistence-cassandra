...
object CassandraSnapshotStoreConfiguration {
  lazy val config = ConfigFactory.parseString(s"""
       ...
       pekko.persistence.cassandra.snapshot.keyspace=CassandraSnapshotStoreSpecSnapshot
...
class CassandraSnapshotStoreSpec
  ...
  val snapshotSettings =
    new SnapshotSettings(system, system.settings.config.getConfig("pekko.persistence.cassandra"))
  ...
  "A Cassandra snapshot store" must {
    "insert Cassandra metrics to Cassandra Metrics Registry" in {
      ...
      metricsNames should contain("pekko.persistence.cassandra.CassandraSnapshotStoreSpec.bytes-sent")
      metricsNames should contain("pekko.persistence.cassandra.CassandraSnapshotStoreSpec.cql-requests")
    }
