...
package object query {
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] val firstBucketFormat = "yyyyMMdd'T'HH:mm"

  /** INTERNAL API */
  @InternalApi private[pekko] val firstBucketFormatter: DateTimeFormatter =
    DateTimeFormatter.ofPattern(firstBucketFormat).withZone(ZoneOffset.UTC)

  /** INTERNAL API */
  @InternalApi private[pekko] def isExhausted(rs: AsyncResultSet): Boolean = {
    rs.remaining() == 0 && !rs.hasMorePages
  }

  /** INTERNAL API */
  @InternalApi private[pekko] def uuid(timestamp: Long): UUID = {
