 ...
 * INTERNAL API
 */
@InternalApi private[pekko] object EventsByTagSettings {
  sealed trait Period
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] class EventsByTagSettings(system: ActorSystem, config: Config) {
...
/** INTERNAL API */
@InternalApi private[pekko] case class TableSettings(
    name: String,
...
/** INTERNAL API */
@InternalApi private[pekko] sealed trait BucketSize {
  val durationMillis: Long
}

/** INTERNAL API */
@InternalApi private[pekko] case object Day extends BucketSize {
  override val durationMillis: Long = 1.day.toMillis
}

/** INTERNAL API */
@InternalApi private[pekko] case object Hour extends BucketSize {
  override val durationMillis: Long = 1.hour.toMillis
}

/** INTERNAL API */
@InternalApi private[pekko] case object Minute extends BucketSize {
 ...
 */
@InternalApi private[pekko] case object Second extends BucketSize {
  override val durationMillis: Long = 1.second.toMillis
}

/** INTERNAL API */
@InternalApi private[pekko] object BucketSize {
